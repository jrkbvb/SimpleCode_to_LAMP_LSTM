...
def create_SC_input_file(SChandle):
	...
	if path.is_file():
		...
		return #Step 1
	height1, period1, angle1, height2, period2, angle2, shipspeed, realizationIndex = parse_handle(SChandle) #Step 2
	# Step 3 below
...
def run_SC_input_file(SChandle):
	...
	command = "simplecode " + "SimpleCode_files\\"+SChandle[:-8] + " "+ realizationIndex + " 1"
	os.system(command)
...
for line in lines:
	...
	if foundStars==True and counter==1:
		...
		if line[-1]=="\n":
			SChandle = SCprefix + line[:-1] #again, to exclude the line break character
		else:
